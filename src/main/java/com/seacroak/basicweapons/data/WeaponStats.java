...
public record WeaponStats(float damage, float speed, double range) {
    ...
    public WeaponStats withDamage(float modifier) {
        return new WeaponStats(damage + modifier, speed, range);
    }

    public WeaponStats withSpeed(float modifier) {
        return new WeaponStats(damage, speed + modifier, range);
    }
}