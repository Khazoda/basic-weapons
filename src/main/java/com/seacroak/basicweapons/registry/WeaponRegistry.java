...
public class WeaponRegistry {
  ...
  private static Item registerWeapon(String name, ToolMaterial material, WEAPON_TYPE type, WeaponStats stats, WeaponFactory<? extends Item> factory) {
    return addItem(type, material, name, Reggie.register(name, (settings) -> {
      Item.Settings itemSettings = attachDefaultSettings(name, type, material, stats.damage(), stats.speed(), stats.range());
      if (material == ToolMaterial.NETHERITE) itemSettings = itemSettings.fireproof();
      return factory.create(material, stats.damage(), stats.speed(), stats.range(), itemSettings);
    }));
  }
